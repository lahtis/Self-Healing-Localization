...
1. [Core Public API](#1-core-public-api)
2. [Logging Configuration](#2-logging-configuration)
3. [Language Utilities](#3-language-utilities)
4. [GLFM Database Loader](#4-glfm-database-loader)
...
7. [Provider Architecture](#7-provider-architecture)
    - [MyMemory Adapter](#76-mymemory-adapter)
    - [LibreTranslate Adapter](#75-libretranslate-adapter)
    - [DeepL Adapter](#73-deepl-adapter)
    - [Google V2 Adapter](#74-google-cloud-v2-adapter)
    - [Papago Adapter](#77-papago-adapter)
8. [Environment Configuration](#8-environment-configuration)
9. [Metadata and Cache](#9-metadata-and-cache)
10. [Provider Capability Matrix](#10-provider-capability-matrix)
11. [Fallback and AI Audit Design](#11-fallback-and-ai-audit-design)
12. [Quick Start Examples](#12-quick-start-examples)
13. [Self-Healing Features](#13-self-healing-features)
...
| Provider Adapters | MyMemoryAdapter, LibreTranslateAdapter, DeepLAdapter, GoogleV2Adapter, PapagoAdapter |
...
| build_request(request) -> Dict[str, Any] | Builds provider-specific payload. |
...
|---|---|
| DEEPL_API_KEY | DeepL API key (can be set in .env/shl/.env) 
|
...
| Variable | Description |
|---|---|
| GOOGLE_API_KEY | Primary Google Cloud Translation API key |
| GOOGLE_BACKUP_API_KEY | Backup API key for failover |

### Failover

- Try the primary key.
- On access, quota, or availability failure, try the backup key.
- If the backup fails, raise the primary error.
...
Google v2 does not expose the glossary and labels functionality of the advanced V3 API.
...
| Variable | Description |
|---|---|
| LIBRETRANSLATE_URL | Base URL for LibreTranslate instance |
...
| Variable | Description |
|---|---|
| MYMEMORY_EMAIL | Email for higher MyMemory quota |
...
| Variable | Description |
|---|---|
| NAVER_CLIENT_ID | Naver Cloud Papago Client ID |
| NAVER_CLIENT_SECRET | Naver Cloud Papago Client Secret |

### Supported Features

- honorific
- glossary
- formality (mapped to honorific)
...
| 401 / 403 | ProviderAccessError |
| 429 | RateLimitExceededError |
| 400 | InvalidRequestError / LanguageNotSupportedError |
| 500+ | ServiceUnavailableError |
| Timeout | ServiceUnavailableError |

### 7.8 Provider Fallback Architecture
Default Fallback Flow

- DeepL (if API key provided)
- Papago (if API key provided)
- Google Translate v2 (if API key provided)
- LibreTranslate (test mode)
- MyMemory (fallback, always available)
...
SHL uses .env files for secure API key management.
...
| GOOGLE_BACKUP_API_KEY | Google Cloud Translation backup API key |
| NAVER_CLIENT_ID | Naver Cloud Papago Client ID |
| NAVER_CLIENT_SECRET | Naver Cloud Papago Client Secret |
...
GOOGLE_BACKUP_API_KEY=your-backup-key
NAVER_CLIENT_ID=your-naver-client-id
NAVER_CLIENT_SECRET=your-naver-client-secret
LIBRETRANSLATE_URL=https://libretranslate.com
LIBRETRANSLATE_API_KEY=your-lt-key
```

### Automatic Loading

All adapters automatically load environment variables from .env/shl/.env when initialized. You can also pass API keys directly as parameters.
...
# Or pass explicitly
deepl = DeepLAdapter(api_key="your-api-key")
```

## 9. Metadata and Cache
### 9.1 TranslationRequest
...
| honorific | Optional[bool] | Papago-specific polite/honorific style (True = polite). |
...
- If capacity is still exceeded, remove the oldest entry.
...
- * Papago maps formality → honorific

| Provider | Formality | Honorific | Glossary | HTML | Context |
|---|---|---|---|---|---|
| DeepL | + | - | + | + | + |
| Papago | +* | + | + | - | - |
| Google v2 | - | - | - | + | - |
...
### 10.1 Provider API Capabilities

| Provider | Local Registry | Mirror Support | Built-in Failover | .env Support |
|---|---|---|---|---|
| DeepL | - | - | - | + |
| Papago | + | - | - | + |
| Google v2 | + | - | + | + |
...
## 11. Fallback and AI Audit Design
...
# Use in UI
title = engine.ui_text("Guestbook")  # → "Vieraskirja"
save = engine.ui_text("Save")        # → "Tallenna"
empty = engine.ui_text("Empty")      # → "Tyhjennä"
...
## 12.3 Translation with Metadata
```python
...
request = TranslationRequest(
    ...
    source_lang="en",
    target_lang="ko",
    honorific=True,          # → polite Korean
)

result = papago.translate(request)
print(result)
```
...
- PapagoAdapter (Naver Cloud Papago Translation API)
- .env configuration support for all providers
- API key masking in logs
- honorific field in TranslationRequest
- Updated Provider Capability Matrix
- Updated provider fallback priority list
...
End of API Reference — Self-Healing Localization Library v0.2.4
...
* Added PapagoAdapter (Naver Cloud Papago Translation API)
* Added honorific field to TranslationRequest (Tier 2)
* Papago supports honorific/polite style for Korean and Japanese pairs

Environment Configuration:

* Added .env support for all providers
* Added shl.utils.env_loader module
* Added mask_api_key() function for secure logging
* All providers now support both explicit parameters and .env file

Documentation Updates:

* Added Environment Configuration section (Chapter 8)
* Updated Provider Capability Matrix with .env support
* Updated provider fallback priority list
* Added Papago examples
