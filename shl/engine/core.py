...
class LocalizationEngine:
    ...
    def ui_text(self, key, default_value=""):
        ...
        # Get from localizer with base fallback
        text = self._get_text_with_fallback(validated_key)
        
        # Self-healing: if text is missing, translate or use default
        if text is None:
            ...
            # Fallback: save and return default
    ...
    def template(self, key, default="", **kwargs):
        ...
        # Get template with base fallback
        text = self._get_template_with_fallback(validated_key)
        
        # Self-healing with default parameter
        if text is None:
