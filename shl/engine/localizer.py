...
class Localizer:
    ...
    def __init__(
        ...
        logger.debug(
            ...
            "lang=%s, base=%s, base_tag=%s, keys=%s",
            self.lang_code,
            self.base_lang,
            self.base_lang_tag,
            len(self.texts),
    ...
    def _invalidate_cache(self) -> None:
        """Clear cached non-active languages."""
        if self._loaded_langs:
            self._loaded_langs.clear()

            if not self._shutting_down:
                logger.debug(
                    "Loaded language cache cleared"
                )
    ...
    def _load_json_safe(
        ...
        try:
            ...
            if not isinstance(data, dict):
                logger.error(
                    "JSON file is not a dictionary: %s",
                    filepath,
                )
    ...
    def _save_texts(
        ...
        try:
            ...
            try:
                ...
                if not self._shutting_down:
                    logger.debug(
                        "Temporary file cleanup failed: %s",
                        cleanup_error,
                    )
    ...
    def save(self) -> bool:
        """Always save current in-memory text data."""
        saved = self._save()
    ...
    def _atexit_save(self) -> None:
        """Best-effort save during interpreter shutdown."""
    ...
    def close(self) -> bool:
        """Save pending changes and close the localizer."""
        if not self._alive:
            return True
    ...
    def __del__(self):
        """Destructor must not save or log."""
        try:
            self._alive = False
        except Exception:
            pass
    ...
    def _migrate_legacy_file(
        ...
        legacy_texts = self._load_json_safe(
            legacy_path
        )
    ...
    def _load_or_create(self) -> Dict[str, Any]:
        """Load active language or create it from the base language."""
        if os.path.exists(self.lang_file):
            texts = self._load_json_safe(
                self.lang_file
            )
        ...
        if os.path.exists(self.base_file):
            loaded_base = self._load_json_safe(
                self.base_file
            )
    ...
    def L(
        ...
        """Get a key and save it immediately if it is missing."""
    ...
    def get_text(
        ...
        """Get text from a selected language."""
        validated_key = self._validate_key(key)
        ...
        if (
            fallback
            and target_lang != self.base_lang_tag
        ):
            return self._get_text_from_lang(
                validated_key,
                self.base_lang_tag,
            )
    ...
    def _get_text_from_lang(
        ...
        """Load text from active memory or the language cache."""
        try:
            normalized_lang = normalize_full_tag(
                lang_code
            )
    ...
    def set_text(
        ...
        """Set a text value and save it immediately."""
        validated_key = self._validate_key(key)
    ...
    def has_key(self, key: str) -> bool:
        """Return True if active language contains key."""
        validated_key = self._validate_key(key)

        return bool(
            validated_key
            and validated_key in self.texts
        )
